...
def print_cow():
    print(" ___________ ")
    print(" \033[07m  marselian.py! \033[27m                # \033[07mC\033[27mommon")
...
def version():
    ...
    print("\r\n	\033[1;31m[ marselian.py ]  " + __version__ + "\033[1;m\r\n")
    print("	* Hacked up by VİYKLL - Muhammedenbussines@gmail.com")
    print("	* http://officialx-marselian.ml\r\n")
...
def improve_dictionary(file_to_open):
    ...
    print("      *      Bazılarında büyük kelime listeleri kullanma  *")
    print("      *       Aşağıdaki seçenekler önerilmez!      *")
    print("      *************************************************\r\n")

    conts = input(
        "> Kelime listesindeki tüm kelimeleri birleştirmek istiyor musunuz? Y/[N]: "
    ).lower()
    ...
    leetmode = input(">Leet modu mu? (yani leet = 1337) Y/[N]: ").lower()
    ...
    if leetmode == "y":
        for (
            ...
            unique_lista
        ):  # if you want to add more leet chars, you will need to add more lines in marselian.cfg too...
    ...
    print_to_file(file_to_open + ".marselian.txt", unique_list_finished)
...
def interactive():
    ...
    profile["isim"] = str(isim)

    profile["Soyadı"] = input("> Soyadı: ").lower()
    profile["nick"] = input("> Takma-ad: ").lower()
    birthdate = input("> Birthdate (DDMMYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] Doğum günü için 8 basamak girmelisiniz!")
        birthdate = input("> Birthdate (DDMMYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("> Ortaklar) isim: ").lower()
    profile["wifen"] = input("> Ortaklar) Takma-ad: ").lower()
    wifeb = input("> Ortaklar) birthdate (DDMMYYYY): ")
    ...
    profile["kid"] = input("> çocuğun isim: ").lower()
    profile["kidn"] = input("> çocuğun Takma-ad: ").lower()
    kidb = input("> çocuğun birthdate (DDMMYYYY): ")
    ...
    profile["pet"] = input("> Evcil Hayvanlar isim: ").lower()
    profile["Şirket"] = input("> Şirket isim: ").lower()
    print("\r\n")
    ...
    if words1 == "y":
        words2 = input(
            "> Lütfen kelimeleri virgülle ayırarak giriniz. [yani hacker,meyve suyu,siyah], boşluklar kaldırılacak: "
        ).replace(" ", "")
    ...
    profile["leetmode"] = input(">Leet modu mu? (yani leet = 1337) Y/[N]: ").lower()
...
def generate_wordlist_from_profile(profile):
    ...
    isimup = profile["isim"].title()
    Soyadıup = profile["Soyadı"].title()
    ...
    rev_n = [rev_isim, rev_isimup, rev_nick, rev_nickup]
    ...
    kombinaac = [profile["pet"], petup, profile["Şirket"], Şirketup]
    ...
    print_to_file(profile["isim"] + ".txt", unique_list_finished)
...
def alectodb_download():
    ...
    passwords = []
    for row in data:
        userisims.append(row[5])
        passwords.append(row[6])
    gus = list(set(userisims))
    gpa = list(set(passwords))
    gus.sort()
    gpa.sort()

    print(
        "\r\n[+] Exporting to alectodb-userisims.txt and alectodb-passwords.txt\r\n[+] Done."
    )
    f = open("alectodb-userisims.txt", "w")
...
def download_wordlist():
    ...
    print("    11   dictionaries    24      isims           37      yiddish")
...
def download_wordlist_http(filedown):
    ...
    arguments = {
        ...
        13: (
            "finnish",
            ("finnish.gz", "firstisims.finnish.gz", "words.finnish.FAQ.gz"),
        ),
        14: ("french", ("dico.gz",)),
        15: ("german", ("deutsch.dic.gz", "germanl.gz", "words.german.gz")),
        16: ("hindi", ("hindu-isims.gz",)),
    ...
    if intfiledown in arguments:
        ...
        print("[+] kaydedilen dosyalar " + dire)
...
# the main function
def main():
    """Command-line interface to the marselian utility"""

    read_config(os.path.join(os.path.dirname(os.path.realpath(__file__)), "marselian.cfg"))
...
def get_parser():
    ...
    group.add_argument(
        ...
        dest="alecto",
        action="store_true",
        help="Parse default userisims and passwords directly"
